...
1. **注册/登录** — JWT Token + Redis黑名单
2. **浏览商品** — 分类筛选 + ES关键词搜索
3. **购物车** — 本地存储 + 数量管理
4. **下单** — Redisson分布式锁扣库存 + Seata AT分布式事务
5. **支付** — 模拟支付 + RocketMQ通知订单服务
6. **订单管理** — 状态流转 + 取消/确认收货
...
| 重复支付 | 数据库UNIQUE KEY `(order_no, pay_type)` 硬约束 |
| 库存与订单不一致 | Seata AT自动回滚 |
| 取消订单库存加不回 | 重试3次+1秒间隔，全部失败回滚取消 |
| 超时取消库存加不回 | 重试3次，失败记录日志 |
